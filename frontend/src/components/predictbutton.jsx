...
const PredictButton = ({ startDate, endDate, selectedStock, stockData }) => {
  // Ensure all required data is available before rendering the button
  const isDataReady =
    startDate && endDate && selectedStock && stockData && stockData.length > 0;
  ...
  return (
    <div>
      {isDataReady ? (
        ...
        <button
          className="text-white bg-gray-400 cursor-not-allowed font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-12"
          disabled
        >
          Predict My Stock
        </button>
