...
const StockDataTable = ({ stockData }) => {
  ...
  const indexOfMaxHigh = stockData.reduce(
    (maxIndex, currentValue, currentIndex, array) =>
      currentValue.high > array[maxIndex].high ? currentIndex : maxIndex,
    0
  );

  const indexOfMinHigh = stockData.reduce(
    (minIndex, currentValue, currentIndex, array) =>
      currentValue.high < array[minIndex].high ? currentIndex : minIndex,
    0
  );
    ...
    <div>
      ...
      <div className="max-h-96 overflow-y-auto my-10">
        <table className="w-3/4 relative mx-auto my-5 text-sm text-left rtl:text-right text-gray-500 border border-collapse table-fixed">
          <thead>
            <tr>
              <th className="px-6 py-3 text-center rounded-tl-2xl rounded-tr border-b bg-gray-100 font-bold ">
                Sr. No
              </th>
              <th className="px-6 py-3 text-center border-b bg-gray-100 font-bold">
                Date
              </th>
              <th className="px-6 py-3 text-center border-b bg-gray-100 font-bold">
                Open
              ...
              <th className="px-6 py-3 text-center rounded-tr-2xl border-b bg-gray-100 font-semibold">
                Close
              </th>
            </tr>
          </thead>
